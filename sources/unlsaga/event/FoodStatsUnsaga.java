...
public class FoodStatsUnsaga extends FoodStats{
	...
	private final int perHealBase = 70;
	...
	public void onUpdate(EntityPlayer par1EntityPlayer)
	{
		...
		if (par1EntityPlayer.worldObj.getGameRules().getGameRuleBooleanValue("naturalRegeneration") && this.foodLevel >= 18 && par1EntityPlayer.shouldHeal())
		{
				...
				for(ItemStack is:pdata.getAccessories()){
					if(is!=null){
						if(is.getItem() instanceof IUnsagaMaterialTool){
							...
							amountHeal += Unsaga.abilityManager.getInheritHealAmount(im.getCategory(), material, Unsaga.abilityManager.healUps);
							amountHeal += Unsaga.abilityManager.getInheritHealAmount(im.getCategory(), material, Unsaga.abilityManager.healDowns);
						}
			...
			int perHeal = this.perHealBase;
			if(amountHeal>0F){
				perHeal = perHeal - (int)(15.0F * amountHeal);
			}else{
				perHeal =perHeal - (int)(18.0F * amountHeal);
			}

			perHeal = MathHelper.clamp_int(perHeal, 20, 200);
			//Unsaga.debug(this.foodTimer+":"+perHeal,this.getClass());
			if (this.foodTimer >= perHeal)
			{
				Unsaga.debug("回復秒:"+perHeal,this.getClass());
